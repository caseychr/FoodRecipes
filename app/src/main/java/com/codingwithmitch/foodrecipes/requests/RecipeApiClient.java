...
public class RecipeApiClient {
    ...
    public void searchRecipesApi(String query, int pageNumber){
        // Create a new runnable whenever we need to and run it on the executors
        if(mRetrieveRecipesRunnable != null){
            mRetrieveRecipesRunnable = null;
        }
        mRetrieveRecipesRunnable = new RetrieveRecipesRunnable(query, pageNumber);
        // Creation of handler to call runnable task inside of thread pool
        final Future handler = AppExecutors.getInstance().networkIO().submit(mRetrieveRecipesRunnable);

        AppExecutors.getInstance().networkIO().schedule(new Runnable() {
            @Override
            public void run() {
                //if the NETWORK_TIMEOUT occurs use the handler to cancel the task
                handler.cancel(true);
            }
        ...
        public void run() {
            try {
                ...
                if(response.code() == 200){
                    ...
                    if(pageNumber == 1){
                        /**
                         * Setting the response list to the LiveData stream here. If we use postValue() we're setting
                         * it on a background thread here. If we wanted to set it on a non-background thread we would
                         * use setValue(). Since we're inside the Executor thread pool here we use postValue().
                         */
